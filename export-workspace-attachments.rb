...
def get_open_projects(this_workspace)
    query                     = RallyAPI::RallyQuery.new()
    query.workspace           = this_workspace
    query.project             = nil
    query.project_scope_up    = true
...
#
def get_all_workspace_attachments (project)
        query       = RallyAPI::RallyQuery.new()
...
@my_workspace.each_with_index do | this_workspace, count_workspace | #{
    ...
    get_open_projects(this_workspace).each do |project|

      print "\nProject #{project.name} (#{extract_id(project.ref)})"


      # ----------------------------------------------------------------------
      # Get all attachments in the Workspace.
      #
      all_workspace_attachments = get_all_workspace_attachments(project)
